                      ...
                      try {
                        ...
                        const message =
                          err instanceof Error
                            ? err.message
                            : `Unable to process cover image: ${file.name}`
                        pushToast('error', message)
                      }
                    }

                    if (processed.length > 0) {
                      setCoverFiles(prev => [...prev, ...processed])
                      pushToast('success', 'Cover images auto-resized to 1120×640')
                    }
                    e.target.value = ''
                  }}
                />
                    ...
                    <div>
                      <input
                        ...
                        onChange={async e => {
                          ...
                          if (processed.length > 0) {
                            setDrawerVendorDraft(s => ({ ...s, newCoverFiles: processed }))
                            pushToast('success', 'Cover images auto-resized to 1120×640')
                          }
                          e.target.value = ''
                        }}
